...
const UserDashBoard = ({route, navigation}) => {
  ...
  const likeHandler = async (id, likeStatus) => {
    try {
      ...
      if (responseData.status === false) {
        ...
        const updatedProperty = filterData.map(item => {
          if (item.id === id) {
            return {...item, likestatus: likeStatus};
          }
          return item;
        });
        setFilterData(updatedProperty);
      }
    } catch (error) {
  ...
  const updateNotification = useCallback(async () => {
    try {
      let base_url =
        'https://xionex.in/msaken/admin/public/api/update-notification';
