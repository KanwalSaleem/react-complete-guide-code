...
const EditPropertyStep3 = ({route, navigation}) => {
  const postPropertyData = route.params;
  const propertyData = route.params.postPropertyData;
  const postPropertyStep1Data = postPropertyData.postPropertyStep1Data;
  const [deleteVisible, setDeleteVisibile] = useState(false);
  ...
  const addProperty = async data => {
    ...
    // try {
    //   let base_url =
    //     'https://xionex.in/msaken/admin/public/api/update-property';
    //   let uploadData = new FormData();

    //   uploadData.append(
    //     'property_for',
    //     propertyData.data.property_for === 'rent' ? 'rent' : 'sale',
    //   );
    ...
    //   postPropertyData.images.length > 0 &&
    ...
    //   propertyData.data.property_for === 'rent' &&
    //     uploadData.append(
    //       'lease_duration',
    ...
    //     uploadData.append(
    //       'date_pro',
    //       `${JSON.parse(JSON.stringify(postPropertyStep1Data.date))}`,
    //     );
    ...
    try {
      ...
      uploadData.append('property_for', propertyData.data.property_for);
      uploadData.append(
        'p_location',
        postPropertyStep1Data.step1FieldData.propertyAddress,
      );
      ...
      uploadData.append('product_type', postPropertyStep1Data.PropertyType);
      uploadData.append('id', propertyData.data.id);
      uploadData.append('bedrooms', postPropertyStep1Data.bedrooms);
      uploadData.append('bathrooms', postPropertyStep1Data.bathrooms);
      uploadData.append('living_rooms', postPropertyStep1Data.livingrooms);
      uploadData.append('kitchens', postPropertyStep1Data.kitchens);
        ...
        <Modal
          ...
          <View style={styles.titleView}>
            ...
            <Text style={styles.successMessage}>
              {language.yourPropertyHasBeenUpdatedSuccessfullyMessage}
            </Text>
          </View>
          <TouchableOpacity
            activeOpacity={0.6}
    ...
    return (
      <Portal>
        <Modal
          ...
          onDismiss={() => setDeleteVisibile(false)}
          contentContainerStyle={[styles.modalContainer, {width: '80%'}]}>
          <View style={styles.deleteContainer}>
            <TouchableOpacity
              activeOpacity={0.6}
              onPress={() => setDeleteVisibile(false)}
              style={[styles.deleteCloseIcon]}>
              <Icon name="close" size={15} color={colors.grey} />
            </TouchableOpacity>

            <Text style={styles.deleteTitle}>
              {language.areYouSureYouWantToDeletePermanently}
            </Text>
            <View style={styles.deleteButtons}>
              <TouchableOpacity
                ...
                <Text style={[styles.applyText, {color: colors.themeRed}]}>
                  {language.cancel}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
  ...
  useFocusEffect(
    useCallback(() => {
      propertyData.unit_amenities
        ? propertyData.unit_amenities.map(item =>
            setUnit(prev => prev.concat(item.value)),
          )
        : setUnit([]);

      propertyData.comunit_amenities
        ? propertyData.comunit_amenities.map(item =>
            setCommunity(prev => prev.concat(item.value)),
          )
        : setCommunity([]);
      ...
      //       const fixedUnitArrays = fixedUnits.map(item => item.name);

      //       unit.map(item => {
      //         if (fixedUnitArrays.includes(item)) {
      //           //
      //         } else {
      //           fixedUnits.push({id: fixedUnits.length + 1, name: item});
      //         }
      //       });
      // setUnitAminities(fixedUnits)
      ...
      //         const fixedCommunityArrays = fixedCommunities.map(item => item.name);

      //         community.map(item => {
      //           if (fixedCommunityArrays.includes(item)) {
      //             //
      //           } else {
      //             fixedCommunities.push({id: fixedCommunities.length + 1, name: item});
      //           }
      //         });
      //   setUnitAminities(fixedCommunities)
      setUnitid(unitAmenities.length);
      setCommunityId(communityAmenities.length);
    }, []),
  );
  ...
  return isLoading ? (
    ...
    <ScrollView
      ...
      <View>
        ...
        <TouchableOpacity
          activeOpacity={0.6}
          onPress={() => setDeleteVisibile(true)}
          style={[
            styles.applyButton,
            {
              backgroundColor: colors.themeWhite,
...
export default EditPropertyStep3;
