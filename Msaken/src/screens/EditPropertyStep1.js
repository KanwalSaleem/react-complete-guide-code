...
const EditPropertyStep1 = ({route, navigation}) => {
  const propertyDetails = route.params.postPropertyData.data;
  ...
  const selectEditLocation = event => {
    const latitude = parseFloat(propertyDetails.latitude);

    const longitude = parseFloat(propertyDetails.longitude);
  ...
  const onSubmit = data => {
    autoAddress &&
      navigation.navigate(
        ...
        {
          postPropertyData: route?.params?.postPropertyData,
          step1FieldData: data,
  ...
  useFocusEffect(
    useCallback(() => {
      if (route.params?.reset) {
        selectEditLocation(), setPropertyType(propertyDetails.product_type);

        setDate(new Date(dayjs(propertyDetails.date_pro).format('YYYY-MM-DD')));

        reset({
          propertyName: propertyDetails.name,
          propertyAddress: propertyDetails.p_location,
          propertyPrice: propertyDetails.price.toString(),
          rentPrice: propertyDetails.price.toString(),
          securityDeposit:
            propertyDetails.property_for === 'rent' &&
            propertyDetails.security_deposit === 'undefined'
              ? language.none
              : propertyDetails.security_deposit,

          landArea: propertyDetails.land_area.toString(),
          streetWidth: propertyDetails.street_width.toString(),
          description: propertyDetails.description,

          buildingDirection: propertyDetails.b_direction,
          realEstateAge: propertyDetails.real_estage_age,
          leaseDuration:
            propertyDetails.property_for === 'rent'
              ? propertyDetails.lease_duration
              : '',
        });
        ...
        beds[propertyDetails.bedrooms - 1] = true;
        setSelectedBedroom(beds);
        baths[propertyDetails.bathrooms - 1] = true;
        setSelectedBathrooms(baths);
        kitchensChecked[propertyDetails.kitchens - 1] = true;
        setSelectedKitchens(kitchensChecked);
        livings[propertyDetails.living_rooms - 1] = true;
        setSelectedLivingrooms(livings);
        setBedrooms(propertyDetails.bedrooms);
        setBathrooms(propertyDetails.bathrooms);
        setKitchen(propertyDetails.kitchens);
        setLivingrooms(propertyDetails.living_rooms);
      ...
      navigation,
      propertyDetails.b_direction,
      propertyDetails.bathrooms,
      propertyDetails.bedrooms,
      propertyDetails.date_pro,
      propertyDetails.description,
      propertyDetails.kitchens,
      propertyDetails.land_area,
      propertyDetails.lease_duration,
      propertyDetails.living_rooms,
      propertyDetails.name,
      propertyDetails.p_location,
      propertyDetails.price,
      propertyDetails.product_type,
      propertyDetails.property_for,
      propertyDetails.real_estage_age,
      propertyDetails.security_deposit,
      propertyDetails.street_width,
      reset,
      route.params?.reset,
    ]),
  );
          ...
          {propertyFor === 'rent' && (
            <>
              ...
              <View
                ...
                <TextInput
                  placeholder={language.dateAvailable}
                  name="availableDate"
                  control={control}
                  rules={{required: true}}
                  style={[
