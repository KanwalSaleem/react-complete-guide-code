...
const EditProperty2 = ({route, navigation}) => {
  const postPropertyData = route.params.postPropertyData;
  const postPropertyStep1Data = route.params.postPropertyData.data;
  ...
  const [PostPropertyFloor, setPostPropertyFloor] = useState();
  ...
  const deleteImage = async id => {
    ...
    try {
      let base_url =
        'https://xionex.in/msaken/admin/public/api/delete-image-property';
  ...
  const onSubmit = data => {
    if (images.length + propertyImages.length >= 3) {
      // (docData || videoUrl) &&
      // videoUrl &&
      // (floorData || PostPropertyFloor) &&
      navigation.navigate('editPropertyStep3', {
        postPropertyData: postPropertyData,
        postPropertyStep1Data: route.params,
        step2FieldData: data,
        images: images,
        videoUrl: videoUrl,
        videoData: docData,
      ...
      Alert.alert('', language.uploadAtleastThreeImagesToEditTheProperty);
    }
  };
  const getSingleProperty = useCallback(async () => {
    ...
    try {
      ...
      if (responseData.status === false) {
        ...
        const uploadImages = [];
        responseData?.data.data.multiple_image.map(item => {
          uploadImages.push({uri: item.image, id: item.id});
        }),
          setPropertyImages(uploadImages);
      }
    } catch (error) {
      Alert.alert(error.message);
    } finally {
  ...
  useFocusEffect(
    useCallback(() => {
      getSingleProperty();
      // const uploadImages = [];
      // postPropertyStep1Data.multiple_image.map(item => {
      //   uploadImages.push({uri: item.image, id: item.id});
      // }),
      //   setPropertyImages(uploadImages);

      setVideoUrl(
        postPropertyStep1Data.video_url === 'null'
          ? ''
          : postPropertyStep1Data.video_url,
      );
      setDocData(postPropertyStep1Data.video);

      setPostPropertyFloor(postPropertyStep1Data.floorplan);

      reset({
        name: postPropertyStep1Data.contact_name,
        email: postPropertyStep1Data.contact_email,
        phone: postPropertyStep1Data.contact_phone,
      });
    }, [
      postPropertyStep1Data.contact_email,
      postPropertyStep1Data.contact_name,
      postPropertyStep1Data.contact_phone,
      postPropertyStep1Data.floorplan,
      postPropertyStep1Data.video,
      postPropertyStep1Data.video_url,
      reset,
    ]),
  );
        ...
        <View style={{marginTop: 20}}>
          ...
          <View style={styles.mediaContainer}>
              ...
              <View style={[styles.videoContainer, {width: '50%'}]}>
                ...
                {docData?.uri ? (
                  ...
                  <TouchableOpacity
                    ...
                    // onLongPress={() => setDocData()}
                  >
                    <Image style={styles.videoImage} source={videoImage} />
                  </TouchableOpacity>
                ) : (
                  <TouchableOpacity
                    onPress={videoPickerhandler}
                  ...
                  propertyImages.map((item, index) => {
                    return (
                      <TouchableOpacity
                        ...
                        onPress={deleteImage.bind(this, item.id)}>
                        <Image
                          source={{
                            uri: `https://xionex.in/msaken/admin/public/uploads/products/${item.uri}`,
                          }}
                          style={styles.fileImage}
                        />
                      </TouchableOpacity>
            ...
            <View style={styles.videoContainer}>
              ...
              ) : PostPropertyFloor?.includes('pdf') ||
                floorData?.type.includes('pdf') ? (
                <TouchableOpacity
                  style={[
                    styles.fileView,
                    selectedLanguage === 'arabic' && {
                ...
                <TouchableOpacity
                  ...
                  <Image
                    ...
                    source={{
                      uri: floorData?.uri
                        ? floorData?.uri
                        : `https://xionex.in/msaken/admin/public/uploads/products/${PostPropertyFloor}`,
                    }}
                  />
                </TouchableOpacity>
              )}
