...
import CompaniesUserList from '../CompaniesUserList';
...
export default function CompaniesList() {
  ...
  const handleFilterByName = (event) => {
    ...
    if (event.target.value) {
      setCompaniesList(
        companiesList.filter((item) => item.title.toLowerCase().includes(event.target.value.toLowerCase()))
      );
    } else {
      setCompaniesList(searchData);
    }
  };
  ...
  return (
    <Page title="User: Account Settings |  ">
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <HeaderBreadcrumbs
          heading={listType === 'card' ? 'Company Cards' : 'Company List'}
          links={[
            { name: 'Dashboard', href: PATH_DASHBOARD.root },
            { name: 'Network', href: PATH_DASHBOARD.network.contacts },
            { name: listType === 'card' ? 'Company Cards' : 'Company List' }
          ]}
          action={
            <>
              <IconButton
        ...
        {listType === 'card' ? (
          <Grid container spacing={3}>
            {companiesList.map((project, index) => (
              <Grid
                key={project._id}
                item
                xs={12}
            ...
            {!companiesList.length && SkeletonLoad}
          </Grid>
        ) : (
          <>
            {companiesList.length > 0 && (
              <CompaniesUserList userList={companiesList} getCompanyList={GetCompaniesList} />
            )}
          </>
        )}
      </Container>
